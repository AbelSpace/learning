...
1. 代码缩进使用4个空格（或者2个空格，根据项目情况统一约定，达成共识）缩进替代 `tab` 缩进；
1. 一行代码长度尽量保持80列左右；
1. 如果编辑器支持在文件保存的时候自动删除行末和空行中的空格(注意:要么全部采用，要么全不采用，否则会产生过多的diff信息)，同时保留文件结尾的空行；
1. JS/CSS 最终发布到产品中的时候需要被压缩，以减小静态资源文件大小，提升页面加载速度；
1. JS里声明变量必须加上 `var` 关键字，如果项目支持ES5/6则优先使用`const`和`let`而不建议使用`var`, 并将所有的`const`和`let`变量放在一起，推荐一条语句声明一个变量；
...
1. JS存放路径推荐：公共的js第三方类库放在static/js/common/lib下，jQuery相关类库放在static/js/common/lib/jQplugin下，我们自己开发的公共类库放在static/js/common下；
...
1. 字符串拼接推荐使用模板字符串；
1. 对象定义时，其中的属性和方法推荐优先使用简写方式；其中简写的属性放在对象的头部；
1. 对象或数组的浅拷贝推荐使用扩展运算符（`...`)；
...
1. 如有必要函数定义的时候可以指定默认参数而不是之后在函数体中修改参数，同时默认参数放在函数签名的后面；
1. 函数定义或调用时如果参数较多需要换行可以考虑每个参数一行，并且在最后一个参数后面加`','`；
1. 对于需要使用匿名函数的场景优先考虑箭头函数；
1. 类的定义和扩展使用`class`和`extends`关键字，而不是修改函数原型；
1. 在只有一个`export`项的模块里面，优先使用`export default`而不是其他的命名`export`；
1. 将所有的`import`语句放在文件的开头；
1. 同一个路径的`import`放在一起，根据情况可以每项一行并适当缩进；
1. 在 node.js 环境尽量减少回调嵌套，原则上不超过两级；
...
该文档来自[JavaScript 编码规范指南](https://github.com/hustcer/jStyle/blob/master/README.md)